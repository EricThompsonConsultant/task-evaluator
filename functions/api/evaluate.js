...
// Cloudflare Pages Function — secure API proxy
...
export async function onRequestPost(context) {
  try {
    const { tasks } = await context.request.json();
    ...
    return new Response(JSON.stringify({ error: "Evaluation failed: " + err.message }), {
      status: 500,
      headers: { "Content-Type": "application/json" }
    });
  }
